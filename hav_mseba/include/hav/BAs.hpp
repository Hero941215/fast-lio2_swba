...
int layer_limit = 4;
int layer_size[] = {30, 25, 20, 20, 20};
// float eigen_value_array[3] = {0.0004, 0.0004, 0.0004};
float eigen_value_array[5] = {1.0/16, 1.0/16, 1.0/16, 1.0/16, 1.0/16};
...
public:
  ...
  vector<Eigen::Vector4d> plane_lm;  // 扰动后的平面路标

  void push_all(std::vector<vector<PointCluster>> &vSigOrigs, std::vector<PointCluster> &vFixPoints)
  {
    // 遍历vSigOrigs，并填充sig_vecs
    // 遍历vFixPoints，并填充plvec_voxels，    
    double coe = 0;
    for(int i=0; i<vSigOrigs.size(); i++)
    {
        plvec_voxels.push_back(&vSigOrigs[i]);
        sig_vecs.push_back(&vFixPoints[i]);

        // for(int j=0; j<win_size; j++)
        //   coe += vSigOrigs[i][j].N;      // 最后，这个参数没有使用上面计算的权重
  ...
  void CreateNoisePlaneLandmark(const vector<IMUST> &xs)
  {
    vector<PointCluster> sig_tran(win_size);
    ...
    for(int a=0; a<gps_size; a++)
    {
      ...
      Eigen::Vector3d vBar = sig.v / sig.N;                          // 均值
      Eigen::Matrix3d cmt = sig.P/sig.N - vBar * vBar.transpose();   // 协方差

      Eigen::SelfAdjointEigenSolver<Eigen::Matrix3d> es(cmt);
      ...
      double min_eigenval = 1e10;
			...
			Eigen::Vector3d normal = es.eigenvectors().col(min_i);
			normal = normal / normal.norm();
			double d = -normal.dot(vBar);

      Eigen::Vector4d pi(normal(0), normal(1), normal(2), d);
      plane_lm.push_back(pi);
      // std::cout << "normal.norm(): " << normal.norm() << " pi: " << normal(0) << " " << normal(1) << " " << normal(2) << " " << pi(3) << std::endl;
    }
  }
  ...
  //       ok_frame++;
  //     }
  //     // std::cout << "ok_frame: " << ok_frame << std::endl;
  ...
  //     // std::cout << "compute one lm time: " << t_one_lm.toc() << " ms" << std::endl;
  //   }
...
public:
  ...
  bool judge_eigen(int win_count)
  {
    ...
    // PointCluster covMat; covMat.clear();
  ...
  void to_margi(int mg_size, vector<IMUST> &x_poses, int win_count)
  {
    if(octo_state != 1)
    {
      ...
      for(int i=0; i<mg_size; i++)
      {
        ...
        vec_fix.insert(vec_fix.end(), vec_tran[i].begin(), vec_tran[i].end());  // 存储固定3d点这个步骤感觉比较耗时
      }
...
public:
  ...
  void multi_second(vector<IMUST> &x_stats, Eigen::MatrixXd &Rcov, VOX_HESS &voxhess)
  {
    ...
    // if(Rcov.determinant()!=0)
    //     std::cout << "can inv" << std::endl;
    // else
    // {
    //   std::cout << "can not inv" << std::endl;
    // }
  ...
  void damping_iter(int iterations, vector<IMUST> &x_stats, VOX_HESS &voxhess, Eigen::MatrixXd &Rcov, int covEnable = 1)
  {
    TicToc t_be_opt;
    double u = 0.01, v = 2;
