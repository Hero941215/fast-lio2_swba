...
class ImuProcess
{
 public:
  ...
  V3D cov_acc_scale;     // 预设值
  V3D cov_gyr_scale;     // 预设值
...
{
  init_iter_num = 1;
  Q = process_noise_cov();                        // 调用use-ikfom.hpp里面的process_noise_cov完成噪声协方差的初始化 12*12？
...
void ImuProcess::IMU_init(const MeasureGroup &meas, esekfom::esekf<state_ikfom, 12, input_ikfom> &kf_state, int &N)
{
  ...
  for (const auto &imu : meas.imu)
  {
    ...
    cur_acc << imu_acc.x, imu_acc.y, imu_acc.z;   // 加速度测量作为初始化均值
  ...
  // 这一步操作很棒，就是相当于认为系统总是从静止开始启动，因此，重力实际上就是反的加速度测量。
  init_state.grav = S2(- mean_acc / mean_acc.norm() * G_m_s2);    // 从common_lib.h中拿到重力，并与加速度测量均值的单位重力求出S2的旋转矩阵类型的重力加速度 

  std::cout << "init grav: " << init_state.grav[0] << " " << init_state.grav[1] << " " << init_state.grav[2] << std::endl;

  if(1)
  {
    V3D init_ang(Log(init_state.rot.toRotationMatrix()));
    std::cout << "init_imu pose t axis: " << init_state.pos(0) << " " << init_state.pos(1) <<  " " << init_state.pos(2) << " " << 
                init_ang(0) << " " << init_ang(1) << " " << init_ang(2) << std::endl;
  }
  ...
  init_P.setIdentity();
  init_P(6,6) = init_P(7,7) = init_P(8,8) = 0.00001;       // v
...
void ImuProcess::UndistortPcl(const MeasureGroup &meas, esekfom::esekf<state_ikfom, 12, input_ikfom> &kf_state, PointCloudXYZI &pcl_out)
{
  ...
  for (auto it_imu = v_imu.begin(); it_imu < (v_imu.end() - 1); it_imu++)
  {
    ...
    if (tail->header.stamp.toSec() < last_lidar_end_time_)    continue;  // 跳过不在当前激光帧中的imu测量
    ...
    // ! 加速度处理？？
    // std::cout << "G_m_s2 / mean_acc.norm(): " << G_m_s2 / mean_acc.norm() << std::endl;
  ...
  for (auto it_kp = IMUpose.end() - 1; it_kp != IMUpose.begin(); it_kp--)
  {
    ...
    for(; it_pcl->curvature / double(1000) > head->offset_time; it_pcl --)
    {
      ...
      // !! ma: (R_i * (imu_state.offset_R_L_I * P_i + imu_state.offset_T_L_I) + T_ei) 是将lidar畸变点先变换到imu系，然后，变换到地图系
...
void ImuProcess::Process(const MeasureGroup &meas,  esekfom::esekf<state_ikfom, 12, input_ikfom> &kf_state, PointCloudXYZI::Ptr cur_pcl_un_)
{
  ...
  if (imu_need_init_)
  {
    ...
    IMU_init(meas, kf_state, init_iter_num);  // 理论上只执行一次
